...
class Whatsapp:
	...
	def __init__(self, whatsapp_url):
		...
		with open(os.getcwd() + "\\user_data.json") as json_file:
		...
		self.image_folder = os.path.join(current_folder_path, 'Image')
		...
		for user_data in self.data.keys():
			if not os.path.exists(os.path.join(self.image_folder,
			                                   (self.data[user_data]['User'] + '_' +
			                                    self.data[user_data]['Data Description']))):
				os.makedirs(os.path.join(self.image_folder,
				                         (self.data[user_data]['User'] + '_' +
				                          self.data[user_data]['Data Description'])))
		os.chdir(current_folder_path)
	...
	def send_message_and_image(self):
		image_folder_list = os.listdir(self.image_folder)
		image_folder_path_area = []
		for i in range(len(image_folder_list)):  # Count number of folder in image folder
			image_folder_path_area.append(os.path.join(self.image_folder, image_folder_list[i]))
		...
		for user_data in self.data.keys():
			...
			if os.listdir(os.path.join(self.image_folder, (self.data[user_data]['User'] + "_" +
			                                               self.data[user_data]['Data Description']))):
				for image in os.listdir(os.path.join(self.image_folder,
				                                     (self.data[user_data]['User'] + "_" +
				                                      self.data[user_data]['Data Description']))):
	...
	def delete_image_folder_buffer(self):
		# Delete all image in image buffer folder
		# Prepare for next run with new image in image buffer folder
		for i in range(len(os.listdir(self.image_folder))):
			image_folder_path = os.path.join(self.image_folder, os.listdir(self.image_folder)[i])
